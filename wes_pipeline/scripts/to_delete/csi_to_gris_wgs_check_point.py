...
### Wen updated Oct 2021
###Only use spaces as indentation. NO TABS!
...
def query_bsi(ids, fields, query_field):
    ...
    #fields is columns to display, search_field is the field to seach against
    
    return bsi

def check_ppl_come_back(received,batch_num):
    '''
this function checks if everyone sent in this back came bk 
    '''

    ptips_in_this_batch = received['Phenotips_ID'].to_list()
    batch_name = 'BATCH' + str(batch_num)
    #print('batch_name: ',batch_name)
    if int(batch_num) < 100: 
        bsi_fields = ['Phenotips ID', 'Batch Sent Exome','Batch Received Exome',
                  'DLM LIS Number', 'Exome ID','CRIS Order Status', 'CRIS Order #', 'Archive',
                  'Active Status', 'Phenotips Family ID','Proband test','Sex']
        suppose_batch = query_bsi(batch_name, bsi_fields, 'Batch Sent Exome')
    else:
        bsi_fields = ['Phenotips ID', 'Batch Sent Genome', 'Batch Received Genome',
                  'DLM LIS Number', 'Exome ID', 'Genome ID', 'CRIS Order Status', 'CRIS Order #', 'Archive',
                  'Active Status', 'Phenotips Family ID', 'Date of Enrollment','Proband test','Sex']
        suppose_batch = query_bsi([batch_name], bsi_fields, 'Batch Sent Genome')
        suppose_batch.to_csv('suppose_batch.csv',index = False)
        
    if ptips_in_this_batch != suppose_batch['Phenotips_ID'].to_list():
        #print('here')
        didnt_come_bk = set(suppose_batch['Phenotips_ID'].to_list())-set(ptips_in_this_batch)
        extra_ptip = set(ptips_in_this_batch) -  set(suppose_batch['Phenotips_ID'].to_list())
        if len(list(didnt_come_bk)) > 0:	    
            print('The following person was sent in the batch, but didnt come back in this batch...Double check.')
            print(didnt_come_bk)
        if len(list(extra_ptip)) > 0 :
            for ex_ptip in list(extra_ptip):
                print('There are extra patient bk with the batch (might be rapidtat):'+ ex_ptip)
	        

    return list(didnt_come_bk),list(extra_ptip)

# For Whole Genome Data:
# Reads in IDs from HGSC delivery files and gets family information
def get_all_family(received,batch_num,didnt_bk):
    '''
and make a ped for everyone in the batch
    '''
    ...
    #in this case: using DSM as id, searching for a list of family-ptip
    ...
    bsi_fields = ['Phenotips ID', 'Batch Sent', 'Batch Received', 'Batch Sent Genome', 'Batch Received Genome',
                  'DLM LIS Number', 'Exome ID', 'Genome ID', 'CRIS Order Status', 'CRIS Order #', 'Archive',
                  'Active Status', 'Phenotips Family ID', 'Date of Enrollment','Proband test','Sex','Father PhenotipsId','Mother PhenotipsId']
    received_and_fam = query_bsi(family_ids, bsi_fields, 'Phenotips Family ID')
    #in this case: using family as id, searching for things in bsi_fields.
    #check for patient didn't come back with the batch.
    #received_and_fam.to_csv('received_and_fam.csv')
    #to fix here
    
    #drop ppl didn't come bk
    if len(didnt_bk) > 0:
        received_and_fam = received_and_fam.drop(received_and_fam[received_and_fam['Phenotips_ID'].isin(didnt_bk)].index)		
    # Clean up BSI data
...
#     ppl_should_be_in_batch = received_and_fam[received_and_fam['Batch_Received_Genome'] == genome_batch]['Phenotips_ID'].to_list()
#     for each_sent in ppl_should_be_in_batch:
#     	if each_sent not in received['Phenotips_ID']:
#     		print('\nThe following person was sent in the batch, but didnt come back in this batch...Double check.')
#     		print(each_sent)
#     		
    ...
    received = received[['Exome_ID', 'Genome_ID_x', 'Genome_ID_y', 'Phenotips_ID_x', 'Phenotips_ID_y', 'DLM_LIS_Number',
                         ...
                         'CRIS_Order#', 'Phenotips_Family_ID', 'Date of Enrollment', 'File_Prefix',"Proband test",'Sex','Father_Phenotips_ID','Mother_Phenotips_ID']]
    received['Notes'] = ''
    ...
    #check for missing probands, still need some work
    print('\nchecking if probands in batch...')
    fam_list = received['Phenotips_Family_ID'].to_list()
    number_of_families = len(fam_list)
    received_and_fam.loc[received_and_fam['Phenotips_ID'].isin(received['Phenotips_ID'].to_list()),'Batch_Received_Genome'] = 'BATCH'+str(batch_num)
    check_df= received_and_fam[received_and_fam['Batch_Received_Genome'].str.contains('BATCH', case = False, na = False)]
    for each_fam in fam_list:
    	this_fam_df = check_df[check_df['Phenotips_Family_ID'] == each_fam]
    	#make sure proband is here
    	proband_col = this_fam_df['Proband test'].to_list()
    	if 'Yes' not in proband_col:
    		#this case proband isn't in db
    		print('\nThe following familys proband not in batch... Double check')
    		print(each_fam)
    		print()
    	elif proband_col.count("Yes") > 1:
    		if len(set(this_fam_df[this_fam_df['Proband test'] == 'Yes']['Phenotips_ID'].to_list())) > 1:
    			print('\nThe following familys have multiple proband in BSI... Double check')
    			print(each_fam)
    			print()
    	else:
    		proband_info = this_fam_df[this_fam_df['Proband test'] == 'Yes']
    		#if "WGS sequenced" in proband["Notes"].to_list() or "WES sequenced" in proband["Notes"].to_list():
    			#pass
    		#elif 'BATCH' + str(genome_batch) in proband_info['Batch_Sent_Exome']:
    			#pass
    		batchnum_list = list(re.findall('[0-9]+',bnum) for bnum in proband_info['Batch_Sent_Genome'])
    		if int(str(batchnum_list[0][0])) > int(genome_batch):
    			print('\nThe following proband is in future batch. Double Check...' )
    			print(proband_info['Phenotips_ID'])
    			print()
	...
	#making ped
    pre_ped = received[['Phenotips_ID', 'Phenotips_Family_ID','Sex','Father_Phenotips_ID','Mother_Phenotips_ID']].copy()
    pre_ped.to_csv('pre_ped.csv',index = False)
    pre_ped['Sex'].mask(pre_ped['Sex'] == 'F', '2', inplace=True)
    pre_ped['Sex'].mask(pre_ped['Sex'] == 'M', '1', inplace=True)
    pre_ped['Sex'].mask(pre_ped['Sex'].isnull(), '-9', inplace=True)
    #pre_ped.loc[pre_ped['Sex'] == 'F', 'Sex'] = 2
    #pre_ped['Father_Phenotips_ID'].mask(pre_ped['Father_Phenotips_ID']=='0', '-9', inplace=True)
    #pre_ped.loc[~pre_ped['Father_Phenotips_ID'].str.startswith('P'), 'Father_Phenotips_ID'] = '-9'
    #pre_ped.loc[~pre_ped['Mother_Phenotips_ID'].str.startswith('P'), 'Mother_Phenotips_ID'] = '-9'
    nines = ['-9']* len(pre_ped)
    pre_ped = pre_ped[['Phenotips_Family_ID','Phenotips_ID','Father_Phenotips_ID','Mother_Phenotips_ID','Sex']]
    pre_ped['affected'] =nines
    #pre_ped.to_csv('BSI_sex_ped.txt',sep='\t', index=False, header=False, mode='a')
    pre_ped.to_csv('BSI_sex_ped.csv',index=False, header=True)
    f = open('BSI_sex_ped.txt','w')
    for i,row in pre_ped.iterrows():
        f.write(str(row.tolist()))
    return received, all_family,number_of_families
...
#received = pd.DataFrame({'File_Prefix': crams,'Phenotips_ID': ,'DLM_LIS_Number': ,'Genome_ID': }) # from actual delivered files
...
def main():
    ...
    lastm_date = os.path.getmtime(directory)
    lastm_date = time.ctime(lastm_date).replace(' ','-')
    ...
    COME_INS = len(received)
    received.to_csv('raw_received.csv',index = False)
    #print('genome_batch',genome_batch)
    didnt_bk , extra_bk = check_ppl_come_back(received,genome_batch)
    
    batch_tracking = ['CSI Batch Title',
 'Vendor',
 'Platform',
 'Vendor Batch Name',
 'Locus folder name',
 'N data received from vendor',
 'Number of families received from vendor',
 'N data not yet received from vendor (evaluate whether this make sense at batch level?)',
 'N data delivered to GRIS',
 'Number of families delivered to GRIS',
 'Date Vendor Data Received']
    ...
    received, all_family,num_fam = get_all_family(received,genome_batch,didnt_bk)
    ...
    if exome_batch:
        ...
        batch_tracking_numbers.append('Batch' + str(exome_batch))
        batch_tracking_numbers.append('Baylor College of Medicine')
        batch_tracking_numbers.append(directory.split('/')[-2])
        batch_tracking_numbers.append('BATCH' + str(exome_batch))
        batch_tracking_numbers.append(COME_INS)
        batch_tracking_numbers.append(num_fam)
        batch_tracking_numbers.append(len(didnt_bk))
        batch_tracking_numbers.append(len(recived))
        batch_tracking_numbers.append(num_fam)
        batch_tracking_numbers.append(lastm_date)
    ...
    if genome_batch:
        ...
        batch_tracking_numbers.append('Batch' + str(genome_batch))
        batch_tracking_numbers.append('Baylor College of Medicine')
        batch_tracking_numbers.append('Illumina')
        batch_tracking_numbers.append(directory.split('/')[-2])
        batch_tracking_numbers.append('BATCH' + str(genome_batch))
        batch_tracking_numbers.append(COME_INS)
        batch_tracking_numbers.append(num_fam)
        batch_tracking_numbers.append(len(didnt_bk))
        batch_tracking_numbers.append(len(wgs_masterkey))
        batch_tracking_numbers.append(num_fam)
        batch_tracking_numbers.append(lastm_date)

    batch_meta_info = pd.DataFrame()
    batch_meta_info['tracking...'] = batch_tracking
    batch_meta_info['numbers'] = batch_tracking_numbers
    batch_meta_info.to_csv('Batch_meta_tracking_' + str(genome_batch) + '.csv', index = False)
